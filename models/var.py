...
from models.basic_var import AdaLNSelfAttn_MoE as AdaLNSelfAttn
...
class VAR(nn.Module):
    def __init__(
        ...
        self.projector = nn.Sequential(
                ...
                nn.SiLU(),
                nn.Linear(4*self.C, 4*self.C),
                nn.SiLU(),
                nn.Linear(4*self.C, 768),
            )
        self.mid_depth = 8 if self.depth >= 16 else self.depth // 2
    ...
    def forward(self, label_B: torch.LongTensor, x_BLCv_wo_first_l: torch.Tensor) -> torch.Tensor:  # returns logits_BLV
        ...
        for i, b in enumerate(self.blocks):
            ...
            if i == self.mid_depth:   
                f_proj = self.projector(x_BLC.float()).to(dtype=main_type)
            ...
            if isinstance(self.word_embed, nn.Linear):
                ...
                for p in self.word_embed.parameters():
                    if p.requires_grad:
                        s += p.view(-1)[0] * 0
                x_BLC[0, 0, 0] += s
        return x_BLC, f_proj    # logits BLV, V is vocab_size
        ...
        for block_idx, sab in enumerate(self.blocks):
            ...
            for sabffn in sab.ffn.experts:
                ...
                if hasattr(sabffn, 'fcg') and sabffn.fcg is not None:
                    ...
                    nn.init.trunc_normal_(sabffn.fcg.weight, std=1e-5)
